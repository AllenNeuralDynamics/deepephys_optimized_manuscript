...
| quantity | meaning |
|---|---|
| p_spike | 0.7535% of R5's fixed validation elements under the scorer-aligned GT support mask |
| resid@spike, off-spike floor | 0.594959 versus 0.592863 on matched channels |
| meaningful dL | $1.58\times10^{-5}$ if all excess spike residual falls to the off-spike floor |
| dL ÷ seed-noise | 3.71× the R5 three-seed loss SD; 25% recovery is approximately one SD |
| zero-residual upper bound | $1.47\times10^{-3}$, but includes unpredictable noisy-target error |
...
| F11 | validation-loss spike headroom and observed loss-delta calibration | §2.4 headroom | new / `validation_loss_headroom` |
