...
| `dprime_raw` | per-unit → mean | raw (undenoised) in-band baseline = 4.497 |
| `auc_raw`, `auc_deep` | per-unit → mean | threshold-free probability that a GT-event score exceeds a background score |
...
contains 18 figure directives; family-specific plots preserve matched bodies, objectives, and budgets.
...
| 6 | template-SNR change versus matched-filter Δd′ | same 31 architecture-comparison entries |
| 7 | width/schedule/depth compute, paired effects, routing, and weak units | ten matched-R5 endpoints |
| 8 | amplitude versus unit quality | same 31 architecture-comparison entries |
| 9 | R0/R1/R5 matched-seed replication | nine recipe endpoints |
| 10–11 | recipe convergence on linear and log-deficit axes | R0–R6 screen |
| 12 | microbatch gradient diagnostics | R8 trajectory |
