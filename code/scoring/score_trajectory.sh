...
#
# Usage:
#   score_trajectory.sh <label> <ckpt_dir>
...
for f in "$CKPT_DIR"/*.pt; do
  ...
  lbl="${LABEL}_${tag}"
  sbatch --job-name="dp_${lbl}" run_ckpt.sbatch      "$f" "$BASE/traj_scores/${lbl}_dprime.csv"
  sbatch --job-name="dg_${lbl}" template_diag.sbatch "$f" "traj_scores/${lbl}"
  n=$((n + 1))
done
