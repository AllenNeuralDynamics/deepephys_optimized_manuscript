...
:::{note}
This is a historical layer-by-layer note for the original `base32`, `omission=1` reference. The
current manuscript schematic is generated by `code/figures/architecture_evolution.py` and saved as
`figures/architecture_evolution.png`; it shows the replicated `base64_om0` R5 body and the R13 NAF
follow-up. Dimensions below should not be read as the current R5/R13 configuration.
:::
