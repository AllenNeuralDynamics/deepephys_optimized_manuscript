...
(10 GT units, `seed=0`). File names use the manifest `label`.
...
For the loss×omission saturation set (champion, omission0, champ_l2, omission0_l2, and the two
SUPPORT-scale runs), also drop one pair per checkpoint named
`<label>_step<STEP>_{dprime,diag}.csv` (12 log-spaced steps). `collate.py` builds the
metric-vs-updates trajectory from these.
